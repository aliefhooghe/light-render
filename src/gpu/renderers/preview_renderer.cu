...
namespace Xrender {
    ...
    preview_renderer::preview_renderer(const bvh_node *device_tree, camera& cam)
    :   gpu_renderer{cam},
        _device_tree{device_tree}
    {
    }

    __host__ void preview_renderer::_call_integrate_kernel(std::size_t sample_count, curandState_t *rand_pool, float3 *sensor)
    {
        preview_integrate_kernel<<<_image_grid_dim(), _image_thread_per_block()>>>(
            _device_tree, _camera, rand_pool, sample_count, sensor);
    }

    __host__ void preview_renderer::_call_develop_to_texture_kernel(const float3 *sensor, cudaSurfaceObject_t texture)
    {
    ...
    // __host__ std::vector<rgb24> preview_renderer::develop()
    // {
    //     // Allocate host image
